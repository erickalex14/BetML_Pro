...
## Summary
- 2097 nodes · 4001 edges · 130 communities (119 shown, 11 thin omitted)
...
### Community 2 - "predicciones_recomendadas"
Cohesion: 0.15
Nodes (14): prediccion_en_vivo(), prediccion_ensemble(), predicciones_hoy(), predicciones_mias(), predicciones_recomendadas(), Session, Predicción combinada — XGBoost + MLP + LSTM, votación ponderada por accuracy de…, Probabilidades EN VIVO — recalcula 1X2 y mercados de gol dado el marcador y… (+6 more)

### Community 3 - "PartidoRepository"
Cohesion: 0.18
Nodes (6): mercados_jugadores(), Mercados individuales de jugador — tiros, tiros al arco, anotar, amarilla,…, PartidoRepository, date, Partido, Session
...
### Community 9 - "lstm.py"
Cohesion: 0.12
Nodes (17): obtener_secuencia_equipo(), Últimos n partidos del equipo (local o visitante, cualquiera de los dos) antes…, cargar_lstm(), CodificadorTemporal, _DatasetSecuencias, entrenar_lstm(), _pad_izquierda(), predecir_lstm() (+9 more)
...
Cohesion: 0.11
Nodes (23): generar_dataset(), DataFrame, ajustar_calibracion(), entrenar_modelo(), guardar_modelo(), DataFrame, guardar_lstm(), cargar_mlp() (+15 more)

### Community 18 - "EstadisticaSofascore"
Cohesion: 0.28
Nodes (7): EstadisticaSofascore, Stats avanzadas del partido desde Sofascore. xG, presiones, duelos, pases…, guardar_jugadores(), guardar_stats_sofascore(), Session, Guarda estadísticas de Sofascore en BD — upsert por partido_id. Antes saltaba…, Guarda estadísticas de jugadores en BD — upsert por (partido_id,…
...
Cohesion: 0.11
Nodes (22): error_no_controlado(), health(), Request, root(), startup(), me(), detalle_partido(), partidos_hoy() (+14 more)
...
### Community 83 - "job_sofascore_en_vivo.py"
Cohesion: 0.12
Nodes (24): Ids de torneo ya aprendidos para esta liga., torneos_conocidos(), correr_job_sofascore_en_vivo(), _estado_desde_evento(), _goles_desde(), _minuto_desde_evento(), _partidos_a_refrescar(), _partidos_hoy_anclados() (+16 more)
...
Cohesion: 0.20
Nodes (10): montecarlo_partido(), Corre simulación Monte Carlo para un partido. Usa xG de Sofascore si está…, estimar_fraccion_restante(), Fracción del partido que falta jugar (0-1), para escalar el xG pre-partido en…, PrediccionService, Partido, Recalcula 1X2 y mercados de gol EN VIVO dado el marcador y minuto actuales — no…, Este servicio orquesta el flujo completo de las predicciones (+2 more)
...
- **Why does `Partido` connect `Partido` to `PartidoRepository`, `ensemble.py`, `calibracion.py`, `PrediccionRepository`, `lstm.py`, `test_parser_imagen.py`, `construir_features_partido`, `job_reentrenar_modelos.py`, `database.py`, `test_alineaciones.py`, `orquestador.py`, `Equipo`, `job_alineaciones.py`, `PartidoService`, `crear_tablas`, `api_client.py`, `fecha_hoy_partidos`, `job_sofascore_en_vivo.py`, `PrediccionService`, `modelos.py`, `test_guardador_no_regresa.py`, `job_odds_sofascore.py`, `resolver_mercado.py`, `prediccion_service.py`, `Odds`, `fusionar_equipos.py`?**
